...
class DatabaseManager:
    ...
    def __init__(
        self,
        db_url="""mongodb+srv://Marius:Lamper42@cluster0.ohahn.mongodb.net/testPur?retryWrites=true&w=majority""",
    ):
        ...
        try:
            ...
            self.db = self.client["testPur"]
            self.runs = self.db["testing_20"]
            print(self.runs)

            log.debug("Ready! Running MongoDB {} on host {}.",)
...
class Consent:
    ...
    def find_buttons(self):
        ...
        denyall = self._findBtnElem(trigs_deny)
        if denyall:
            log.debug("FOUND REJECT BUTTON!")
            denyall = Button(self.url, denyall)
            self.denyall = denyall
            self.denyall_meta = self.denyall.getMeta()
    ...
    def _findMoreLink(self):
        ...
        """
        xpaths = [
            "//a[contains(@href,'cookie')]",
            "//a[contains(@href,'policy')]",
            "//a[contains(@href, 'learn more')]",
        ]
        for xpath in xpaths:
            ...
            if consent:
                ...
                if self.consent.moreBtn:
                    ...
                    if self.consent.moreBtn.redirect:
                        ...
                        if not aldr_at_lvl:
                            ...
                            self.conset = ConsentSettings(self.url, self.consent.elem,)
                ...
                self.db.modify_run(
                    self.runId,
                    {
                        ...
                        "settings_page": self.conset.getMeta(),
